...
def test_acte_medical_birth_form_accorded_both_genres(tmp_path: Path) -> None:
    # R5 (Rafael 2026-07-13) : l'acte medical figeait « né(e) le » inclusif ; le genre CONNU est
    # desormais resolu POUR LES DEUX genres au niveau des tokens -> « né le » (homme) / « née le »
    # (femme), jamais « né(e) ». Supersede le lock ronde 2 (« homme garde la forme inclusive »).
    masc = _docx_text(
    ...
    assert "né le 10 mars 1975" in masc
    assert "née le 10 mars 1975" in fem
    assert "né(e)" not in masc
    assert "né(e)" not in fem

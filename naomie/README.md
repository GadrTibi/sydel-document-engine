# la Chaloupe — pack d'embarquement du Mousse (Naomie)

*Face Capitaine. À lire par Gad, pas par Naomie.*
...
(Naomie)** pilote le *métier* d'un sprint et apprend ; son **second** (le Claude Code de Naomie) fait
...
## La seule ligne à donner à Naomie
> Sur le projet, ouvre Claude Code et tape : `/embarquer`

Rien d'autre. La commande `/embarquer` :
- fait le « **qui va là ?** » corsaire (opérateur = Naomie → **mode Mousse**) ;
...
Naomie débarque seule, en télétravail, sans rien connaître de L'Équipage. Pour qu'elle vive le sprint
...
(Capitaine) : ton normal.** Le pirate s'arme **seulement après** que Naomie a confirmé être le Mousse au
...
Pour savoir où en est le Mousse, demande à ton Claude : **« où en est Naomie ? »**. Il lit les traces
(runtime → worklog → branche `naomie/selas/*` → commits) et répond au **format rapport boss** — pas
besoin d'un compte-rendu oral de Naomie.

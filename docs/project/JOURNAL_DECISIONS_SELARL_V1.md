...
| **SELARL-SCOPE-2** | **RÉACTIVATION SELARL multi-associés** : la SELARL peut **ajouter des membres** (personne morale ET physique, N) ; nb d'associés auto = signataires ; praticien/gérant intégrés ; statuts (comparution « LES SOUSSIGNÉS » + liste, art. 7 apports, art. 8 répartition numérotée « 1° … détenant N parts ; », signatures) s'adaptent. **Implémenté ADDITIF** (commit `9593f56`) : `StatutsSel.membres` ; à 1 membre → uniperso byte-identique (préservé, 20 tests régression verts). **⚠️ Contredit Albane (05/06 « laissons tomber la SELARL à plusieurs associés ») → GO Gad 2026-06-17 + confirmation Albane EN ATTENTE.** **TODO fidélité (parqués, à confirmer Albane)** : libellé exact de la ligne d'apport art. 7 en multi (le ticket V3 ne fige que la répartition art. 8) ; wording de la ligne « Total des apports » multi. | TICKETS V3 (Rafael) + GO Gad 2026-06-17 | appliqué (build) / **open** (confirmation + wording apport multi → Albane) |
...
| **SELARL-LOC-1** | **Descriptif libre du local** (bail) : rempli → inséré tel quel à la place de la phrase type « Les locaux sont composés… » ; vide → phrase **supprimée** (aucune phrase incomplète, jamais bloquant). **Rafael 2026-06-11 : le document concerné = l'acte de cession d'un cabinet médical** (là où vit la phrase, token `[superficie_local]`) ; l'override s'applique aussi aux compromis qui portent la même phrase. | Ticket client 2.5 + Rafael 2026-06-11 | appliqué (confirmé) |
...
| **SELARL-PRET-1** | **Compromis de cession DENTAIRE — taux + durée du prêt** : le modèle source avait un **trou figé** (« au taux maximum de ___ l'an », aucune variable) alors que le compromis médical porte taux + durée. Variables `[taux_pret]` + `[duree_pret]` ajoutées au modèle dentaire, **calquées exactement sur le médical** (« …au taux maximum de [taux_pret] l'an hors assurance, amortissable sur une période minimum de [duree_pret], »). Lock humain explicite (correction de coquille modèle autorisée). Champs facultatifs → vides = zone à compléter (philosophie SELARL-OPT-1). | Rafael 2026-06-11 (« oui fais cela ») | appliqué |

## Lot retours client 2026-06-11 — SECTION 2 « Sur les documents générés » (lock 007)
⚠️ Cette section du mail d'Albane (10/06) **n'avait PAS été transcrite par Rafael** (le ticket 35
critères = section 1 uniquement). Récupérée en direct → `docs/review/selarl_human_returns_007_albane_2026-06-10_raw_v1.md`.

| Code | Décision | Source | Statut |
|---|---|---|---|
| **SELARL-PROC-1** | **Procuration** : modèle d'avril mis à jour — sous l'adresse SYDEL, ajout du **RCS PARIS 788 531 432** (SIREN) + **téléphone 0153814303** (constantes SYDEL, vérifiées via signature David). + aération. | Albane 2026-06-10 (modèle joint) | appliqué |
| **SELARL-STAT-EUR-1** | **Statuts DENTISTE** : « euros » ajouté art. 7 (montant en chiffres ×2 + en lettres) et art. 8. Le bloc **médecin l'avait déjà** (d'où le retour, Albane testant un dentiste). | Albane 2026-06-10 | appliqué |
| **SELARL-STAT-2** | **Statuts** : intitulé du doc = « **Statuts {dénomination}.docx** » ; DENTISTE → « marié avec Mme » remis **après l'adresse du domicile**, **n° d'inscription à l'ordre** ajouté avant le RPPS (si renseigné), « **Société** » majuscule (entête). Mise en page : entête centrée/compacte, nom société art. 3 **gras + centré**, « **Fait à** » à gauche, nom signataire en gras, ligne « Liste des actes accomplis » centrée. | Albane 2026-06-10 | appliqué |
| **SELARL-PV-1** | **PV nomination gérant** : à **UN SEUL associé**, bascule sur le **PV des décisions de l'ASSOCIÉ UNIQUE** (modèle simplifié fourni par Albane) au lieu de l'AG multi-associés — juridiquement, une société à un associé ne tient pas d'AG. Ordre du jour en **tirets**. Les types réellement multi-associés (SCI/SCM/SCS/SELAS) gardent l'AG. Identité robuste (associé sinon dirigeant), profession optionnelle, accord en genre. | Albane 2026-06-10 (modèle joint) | appliqué |
| **SELARL-ORD-1** | **Demande d'inscription à l'ordre** : CP + ville de l'adresse perso sur **2 lignes** ; nom de l'ordre **aligné** avec son adresse ; aération ; **nom du conseiller** (mandataire) adaptable via l'UI (au lieu de Jordan ELBAZ en dur) ; « **Madame la Présidente** » via case à cocher si la présidente de l'ordre est une femme (défaut « Monsieur le Président », champ `ordre_president_feminin`). | Albane 2026-06-10 | appliqué |
| **SELARL-DNC-1** | **Déclaration de non-condamnation** : « né le {date} à {ville} (**{département}**) » — plus de point après la ville, **département entre parenthèses** si renseigné (nouveau `Person.departement_naissance`) ; bloc identité compact + aération sous le cadre. Rappel légal déjà en italique en bas (rien à changer). | Albane 2026-06-10 | appliqué |
| **SELARL-AUTOR-1** | **Autorisation de domiciliation** : police **Roboto 10** sur tout le document. **Open** : le modèle n'a pas de titre distinct → le « titre en 11 » demandé par Albane n'a pas de cible (à confirmer avec elle). | Albane 2026-06-10 | appliqué / **open** (titre → Albane) |
| **SAVE-DOSSIER-1** | **Sauvegarde / reprise de dossier** (réf. dossier) demandée par Albane : **HORS lettre de mission** → parquée au Manifeste, Gad en discute avec son **associé Rafael** avant décision. **Non construit.** | Albane 2026-06-10 / Gad 2026-06-11 | parqué |

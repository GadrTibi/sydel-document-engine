...
Lancer `FRONT-TEST-PREFILL-001` : concevoir les prefills fictifs et deterministes du nouveau front global et du mode unitaire, en s'appuyant sur `front_data` sans transformer les donnees du prototype en source metier. `FRONT-REVIEW-001` reste bloque jusqu'a consolidation des prefills/tests front, et `SELARL-JURIST-REVIEW-001` reste recommande pour la validation metier/juridique du parcours SELARL existant.

...
- L'editeur dossier V1 du nouveau front global est disponible dans `src/sydel_doc_engine/app/front_dossier_editor.py` et branche dans la zone `Dossier` du shell. Il expose profils prudents, `DossierRecord`, etapes/blocs `dossier_flow`, exigences, documents attendus et statuts/lots `document_status`.
- Le prochain ticket front recommande est `FRONT-DOCUMENTS-PANEL-001` : construire le panneau operationnel des documents attendus a partir de l'editeur dossier et de `document_status`, sans toucher aux generateurs ni au moteur DOCX/PDF/ZIP.
...
Lancer `FRONT-DOCUMENTS-PANEL-001` : transformer la vue des documents attendus en panneau operationnel du nouveau front, avec statuts, reserves, documents manuels, raisons de blocage et statut de lot depuis `document_status`, sans toucher aux generateurs ni au moteur DOCX/PDF/ZIP. `SELARL-JURIST-REVIEW-001` reste recommande en parallele pour la validation metier/juridique du parcours SELARL existant.

...
- L'UI Streamlit dispose maintenant de deux modes :
  - `Assistant metier` : selection documentaire pilotee par `get_expected_documents(...)`, conditions metier par famille CASE-CATALOG-001, parcours SELARL pilote branche sur le schema `selarl_form_schema.py`, tableau des documents attendus incluant manuels/non implementes/reserves, generation DOCX filtree sur les documents generables et prets, ZIP avec manifest, PDF local optionnel et telechargements ;

...
Lancer `FRONT-UNIT-DOCUMENT-MODE-001` : reconcevoir le mode document unitaire comme diagnostic separe du parcours dossier complet, en s'appuyant sur le flow et les statuts documentaires sans coder le rebuild UI visible. En parallele, `FRONT-TEST-PREFILL-001` est maintenant debloque et `SELARL-JURIST-REVIEW-001` reste recommande pour la validation metier/juridique du parcours SELARL existant.

...
- Le prochain ticket front recommande est `FRONT-DOSSIER-EDITOR-001` : construire un premier editeur dossier data-first a partir de `DossierRecord`, roles explicites, adresses typees et `dossier_flow`, sans toucher aux generateurs ni au moteur DOCX/PDF/ZIP.
...
Lancer `FRONT-DOSSIER-EDITOR-001` : construire un premier editeur dossier data-first depuis le shell cible, avec `DossierRecord`, roles explicites, adresses typees et validations `front_data`, sans rebrancher encore toute la generation. `SELARL-JURIST-REVIEW-001` reste recommande en parallele pour la validation metier/juridique du parcours SELARL existant.

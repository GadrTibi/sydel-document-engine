...
Lancer `FRONT-DATA-LAYER-001` : creer la couche de donnees front globale a partir des objets et regles V1, en integrant `docs/project/GLOBAL_FRONT_SENTINEL_CHECKS_V1.csv` comme checklist de couverture. Ne pas modifier les generateurs, le moteur DOCX/PDF/ZIP ni le prototype Streamlit. En parallele, `SELARL-JURIST-REVIEW-001` reste recommande pour la validation metier/juridique du parcours SELARL existant.

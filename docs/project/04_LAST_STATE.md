...
- Le shell UI du nouveau front global est disponible dans `src/sydel_doc_engine/app/front_shell.py` et branche dans `src/sydel_doc_engine/app/streamlit_app.py`. Depuis `FRONT-UX-HARD-CUT-001`, la vue normale affiche uniquement `Type de dossier`, `Donnees a saisir` et `Generation`; les outils prototype/test/diagnostic sont derriere la sidebar `Outils internes`.
- L'editeur dossier V1 du nouveau front global est disponible dans la zone `Dossier` du shell. Le profil `SELARL creation simple` saisit maintenant des donnees reelles vers `DossierRecord`, puis recalcule `dossier_flow` et `document_status`.
- Les actions de generation V1 du nouveau front sont disponibles pour `DOC-001` a `DOC-004` sur `SELARL creation simple` : DOCX, ZIP apres DOCX et PDF optionnel selon backend local. `DOC-006`, `DOC-013` et `DOC-014` restent exclus.
...
- Audit `FRONT-REALITY-CHECK-001` : le hard cut est vrai techniquement, mais la surface normale reste trop chargee pour le ressenti utilisateur ; `FRONT-GENERATION-READINESS-UX-001` et `FRONT-DOCUMENTS-PANEL-001` sont a absorber ou repousser pour ne pas rajouter de surface.
- Le prochain jalon front recommande est `FRONT-MINIMAL-SURFACE-CLEANUP-001`, avant tout push, redeploiement ou test utilisateur.
...
- Tickets READY confirmés : `GLOBAL-CANONICAL-V2-ARBITRATION-001`, `CLOSE-PROJECT-V1-001`, `SELARL-JURIST-REVIEW-001`.
...
Lancer `FRONT-MINIMAL-SURFACE-CLEANUP-001` : appliquer la surface normale strictement limitee a `Type de dossier`, `Donnees a saisir` et `Generation`, cacher le debug interne hors session utilisateur, cacher le PDF si le backend local est indisponible et afficher les blocages runtime dans la zone `Generation`. Ne pas lancer `FRONT-DOCUMENTS-PANEL-001` comme panneau visible avant cette coupe. `SELARL-JURIST-REVIEW-001` reste recommande en parallele pour la validation metier/juridique du parcours SELARL existant.

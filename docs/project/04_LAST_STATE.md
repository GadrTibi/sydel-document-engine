...
- Le shell UI du nouveau front global est disponible dans `src/sydel_doc_engine/app/front_shell.py` et branche dans `src/sydel_doc_engine/app/streamlit_app.py`. L'entree visible separe `Nouveau front global` de `Prototype / outils de test`.
...
- Le prochain jalon front recommande est un premier vrai test local du nouveau front, puis `FRONT-DOCUMENTS-PANEL-001` pour construire le panneau operationnel des documents attendus a partir d'un dossier partiellement saisi et de `document_status`, sans toucher aux generateurs ni au moteur DOCX/PDF/ZIP.

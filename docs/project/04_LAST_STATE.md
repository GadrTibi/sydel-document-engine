...
- Les actions de generation V1 du nouveau front sont disponibles pour `DOC-001` a `DOC-004` sur `SELARL creation simple` : DOCX, ZIP apres DOCX et PDF optionnel selon backend local. En surface normale, le PDF est cache lorsque le backend local est indisponible. `DOC-006`, `DOC-013` et `DOC-014` restent exclus.
...
- Le prochain jalon front recommande est un test utilisateur local du pilote `SELARL creation simple`, avant tout panneau documents ou extension documentaire visible.
...
Lancer un test utilisateur local du pilote `SELARL creation simple` sur la surface minimale. Ne pas lancer `FRONT-DOCUMENTS-PANEL-001` comme panneau visible avant ce test. `SELARL-JURIST-REVIEW-001` reste recommande en parallele pour la validation metier/juridique du parcours SELARL existant.

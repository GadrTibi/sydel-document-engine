...
- Le shell UI du nouveau front global est disponible dans `src/sydel_doc_engine/app/front_shell.py` et branche dans `src/sydel_doc_engine/app/streamlit_app.py`. L'entree visible separe `Nouveau front global` de `Prototype / outils de test`; depuis `FRONT-UX-CLEANUP-001`, le nouveau front affiche directement le parcours principal type de dossier / saisie / resume / generation, avec diagnostics replies.
...
- Le prochain jalon front recommande est un premier vrai test local du nouveau front simplifie sur `SELARL creation simple`, puis `FRONT-DOCUMENTS-PANEL-001` seulement si le test confirme le besoin d'un panneau operationnel des documents attendus separe.
...
Faire un premier vrai test local du nouveau front sur `SELARL creation simple` : saisir un dossier V1, verifier `DOC-001` a `DOC-004`, generer les DOCX, creer le ZIP et tester le PDF si le backend local est disponible. Ensuite lancer `FRONT-DOCUMENTS-PANEL-001` pour consolider le panneau Documents attendus avec statuts, reserves, documents manuels, raisons de blocage et statut de lot depuis `document_status`, sans toucher aux generateurs ni au moteur DOCX/PDF/ZIP. `SELARL-JURIST-REVIEW-001` reste recommande en parallele pour la validation metier/juridique du parcours SELARL existant.

...
| :--- | :--- | :--- | :--- |
| SELARL | — (réf, fini) | **livré + mergé main** · re-audit fond+forme ✅ (`_SELARL_FIDELITY_RECHECK_V1.md`) : FOND fidèle (épinglé par test), FORME 95 % · **lot retours client 2026-06-11 livré** (questionnaire + cession pilotée par les données du dossier + salariés 0..N + facultatifs non bloquants ; 35 critères, cf. `JOURNAL_DECISIONS_SELARL_V1.md` § 2026-06-11 ; 417 verts) | durcissements à folder dans le fix systémique : footer médecin (pagination + « Statuts [dénom] ») ; tests dentiste + page de titre ; alignement dentiste (cosmétique) ; **open Rafael : acte médical sans crédit-vendeur (clause figée du modèle)** |

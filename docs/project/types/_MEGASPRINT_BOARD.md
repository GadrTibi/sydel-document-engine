...
| :--- | :--- | :--- | :--- |
| SELARL | — (réf, fini) | **livré + mergé main** · re-audit fond+forme ✅ (`_SELARL_FIDELITY_RECHECK_V1.md`) : FOND fidèle (épinglé par test), FORME 95 % · **lot retours client 2026-06-11 livré** — section 1 (questionnaire + cession pilotée par les données + salariés 0..N + facultatifs non bloquants, 35 critères) ET **section 2 « documents générés »** (procuration MAJ, statuts titre/euros/n° ordre/MEP, **PV associé unique**, demande ordre conseiller/Présidente, DNC né-à/département, autorisation Roboto 10 ; cf. lock 007) ; **421 verts** | durcissements à folder dans le fix systémique : footer médecin (pagination + « Statuts [dénom] ») ; tests dentiste + page de titre ; alignement dentiste (cosmétique) ; **open Rafael : acte médical sans crédit-vendeur (clause figée du modèle)** |

...
| WORKSTREAM-TRACE-BOSS-REPORT-001 | DONE | Corriger le suivi Naomi en tracabilite de flux et rapport boss court | demande Gad 2026-06-02 : ne pas evaluer Naomi personnellement ; tout travail fait dans son perimetre remonte comme flux Naomi ; rapport court et decisionnel | `WORKSTREAM_TRACE_AGENT_PROTOCOL_V1.md` cree ; AGENTS/tour/workflow/handoff/org chart/sprint/worklog/rattrapage alignes sur flux Naomie + rattrapage retroactif |
| NAOMIE-SYNC-CHECKPOINT-001 | IN_PROGRESS | Recuperer l'avancee SELAS annoncee mais absente des traces publiees | Gad 2026-06-02 indique que Naomie a termine SELAS jusqu'a attente retour humain ; branche `codex/naomie-selas-sprint` ne montre que les commits protocole | `NAOMIE_WORKSTREAM_SYNC_PROTOCOL_V1.md` cree ; sprint/worklog/tour/orchestrateur traces : attendre commit pousse ou Sync packet de Naomie avant requalification |
...
| SELARL-FINAL-ASSOCIE-VALIDATION-001 | BLOCKED | Faire valider le pack final par l'associe | pack 005 attendu apres retours humains 006 | validation suspendue : pack 004 a corriger avant cloture |
...
| SELARL-RETURNS-006-STATUTS-001 | READY | Corriger les retours 006 sur statuts SELARL | rapport triage 006 + `DOC-016`/`DOC-017` | mentions matrimoniales, accord associe, annexe page suivante, tiret `Ouverture...`, tests statuts |
| SELARL-RETURNS-006-DNC-001 | READY | Corriger la declaration de non condamnation et le champ `au` ville naissance | rapport triage 006 + `DOC-001` | `Ne le ... a/au ...`, case/preference `au`, tests DNC |
| SELARL-RETURNS-006-PV-001 | READY | Corriger le PV nomination gerant | rapport triage 006 + `DOC-004` | forme juridique redigee + capital social exact, tests PV |
| SELARL-RETURNS-006-PROCURATION-001 | READY | Corriger la procuration | rapport triage 006 + `DOC-003` | phrase `demeurant..., agissant...` conforme, tests procuration |
| SELARL-RETURNS-006-CONJOINT-LETTERS-001 | READY | Corriger les lettres regime communautaire | rapport triage 006 + `DOC-005`/`DOC-006` | adresse conjoint derivee, forme juridique redigee, date renonciation retiree, tests regime |
| SELARL-RETURNS-006-ORDRE-001 | READY | Corriger la demande d'inscription a l'ordre | rapport triage 006 + `DOC-034` | conseil departemental compose depuis profession + departement, tests ordre |
| SELARL-RETURNS-006-FRONT-VARIABLES-001 | READY | Simplifier variables/front SELARL selon retour 006 | rapport triage 006 + clean front Track B | constantes 99 ans / 4 exemplaires / associe / date du jour, nationalite portugaise, reuse siege=adresse perso |
| SELARL-RETURNS-006-ADDRESS-SIGNATURE-001 | READY | Appliquer les regles transversales adresses/signatures du retour 006 | rapport triage 006 + pack SELARL | CP avant ville + suppression encadres signature controles sur pack |
| SELARL-CLOSING-PACK-005 | BLOCKED | Regenerer un nouveau pack SELARL apres corrections 006 | tickets `SELARL-RETURNS-006-*` DONE | `artifacts/selarl_closing_pack_005/` + manifest + rapport pack 005 |
| SELARL-HUMAN-RETURNS-DEEP-AUDIT-006 | BLOCKED | Auditer les retours 006 sur le pack 005 | pack 005 | controle retours 006 OK ou ecarts restants classes |
| SELARL-CANONICAL-CLOSE-001 | BLOCKED | Clore le statut canonique SELARL simple/regime | validation finale associe ou corrections traitees | `SELARL_CANONICAL_STATUS_V1.md` mis a jour avec statut final |
...
| COMPANY-TYPE-SPRINT-PLAYBOOK-002 | DONE | Completer le mode d'emploi reusable avec les apprentissages de cloture SELARL | pack 004 + audit trois sources + retours humains 005 + demande Gad | playbook/orchestrateur/protocoles Naomie alignes : trois sources, questions utiles seulement, pack actif, audit fidelite, retour associe par ecarts concrets, cloture `DONE/PARTIAL/BLOCKED` |
...
| SPRINT-SELAS-V1-001 | IN_PROGRESS | Suivre le sprint SELAS pour Naomie en phase NotebookLM | choix logique Codex valide par Gad + methode SELARL + protocols sprint/reuse | `docs/sprints/SPRINT_SELAS_V1.md` + phase 3 NOTEBOOKLM + Prompt 01 a donner apres identification de Naomie + aucun dev autorise |
...
- Prochaine étape courante : revue manuelle du pack `artifacts/selarl_closing_pack_004/`, puis `SELARL-FINAL-ASSOCIE-VALIDATION-001`.
...
- Prochaine etape recommandee : utiliser `docs/project/SELARL_CANONICAL_STATUS_V1.md`, puis faire valider le pack 004 avant de rouvrir `SELARL-COMPLETE-COMPLEX-SUBFORMS-001`.
...
- Sprint Naomie actif : SELAS, `docs/sprints/SPRINT_SELAS_V1.md`, phase 0 `ACCUEIL / INITIALISATION`, statut `NO-GO dev`.
...
- Pour l'accueil de Naomie apres identification : repondre avec `Statut sprint`, `Action maintenant`, `Point pedagogie`, `Prochaine etape`; aucun dev avant NotebookLM, audit reutilisation, matrice documentaire, tickets et `GO dev` explicite de Gad.
...
- Pour la SELARL : lire `docs/project/SELARL_CANONICAL_STATUS_V1.md`, puis poursuivre `SELARL-FINAL-ASSOCIE-VALIDATION-001` ou choisir un seul sous-cas avec `GO dev` explicite. Aucun ticket de dev SELARL complexe ne doit demarrer sans ce choix.

...
| SPRINT-SELAS-V1-001 | IN_PROGRESS | Suivre le sprint SELAS pour Naomie apres GO dev borne front/data, specs President/Procuration/DNC/Domiciliation/Decision President/Statuts/Capital-Actions/Ordre/Regime communautaire, roadmap A-Z, dev limite DOC-003/DOC-001/DOC-002/DOC-034/DOC-005-DOC-006/DOC-018, readiness pack, orchestration pack, smoke happy path, smoke regime communautaire, anti-regression wording, checkpoint trois sources et pack de revue humaine | choix logique Codex valide par Gad + methode SELARL + protocols sprint/reuse + reponses NotebookLM Prompts 01-07 + suivi sources + audit reuse V1 + matrice V1 + contrat front V1 + tickets V1 + demande arbitrage premier dev + retour Gad reutilisation/cas par cas + `ok go` Gad + schema front/data + spec President + spec Procuration + GO DOC-003 + spec DNC President + GO DOC-001 + spec DOC-002 Domiciliation + GO DOC-002 + spec Decision President + spec Statuts SELAS medecin AU + spec Capital Actions + spec Ordre + spec Regime communautaire + roadmap A-Z + GO DOC-034 + GO DOC-005/DOC-006 + GO DOC-018 + SELAS-FRONT-READINESS-PACK-001 + SELAS-ORCHESTRATOR-PACK-001 + SELAS-SMOKE-HAPPY-PATH-001 + SELAS-SMOKE-REGIME-COMMUNAUTAIRE-001 + SELAS-ANTI-REGRESSION-WORDING-001 + SELAS-TRIPLE-SOURCE-CHECK-001 + SELAS-HUMAN-REVIEW-PACK-001 | `docs/sprints/SPRINT_SELAS_V1.md` + journal NotebookLM structure + livrables sprint + `SELAS-FRONT-SCHEMA-001` code + specs President/Procuration/DNC/Domiciliation/Decision President/Statuts/Capital-Actions/Ordre/Regime communautaire + roadmap A-Z + `DOC-003` SELAS-safe + `DOC-001` DNC President SELAS-safe cote schema/tests + `DOC-002` Domiciliation SELAS-safe cote schema/tests + `DOC-034` Ordre SELAS-safe cote schema/tests + `DOC-005`/`DOC-006` Regime communautaire SELAS-safe cote schema/tests + `DOC-018` Statuts SELAS medecin AU SELAS-safe cote generateur/tests + readiness pack SELAS alignee + orchestrateur pack SELAS aligne + smoke happy path pack simple OK + smoke regime communautaire pack conditionnel OK + anti-regression wording strict OK + checkpoint trois sources partiel OK + pack de revue humaine pret + pack SELAS non valide final |
| SELAS-REUSE-AUDIT-001 | DONE | Auditer la reutilisation SELARL/global pour SELAS en lecture seule | `SPRINT_SELAS_NOTEBOOKLM_LOG_V1.md` + sources SELAS nommees + registres SELARL/global + specs ordre/statuts | `docs/sprints/SPRINT_SELAS_REUSE_AUDIT_001.md` + decisions `identique/reuse-check/adapter/no-go` + questions ouvertes + `NO-GO dev` maintenu |
| SELAS-MATRIX-001 | DONE | Produire la matrice documentaire SELAS provisoire en lecture seule | `SPRINT_SELAS_REUSE_AUDIT_001.md` + catalogue SELAS + specs ordre/statuts + journal NotebookLM | `docs/sprints/SPRINT_SELAS_MATRIX_001.md` + pack candidat SELAS medecin associe unique + exclusions complexes + prochain `SELAS-FRONT-CONTRACT-001` |
| SELAS-FRONT-CONTRACT-001 | DONE | Produire le contrat metier-front SELAS provisoire en lecture seule | `SPRINT_SELAS_MATRIX_001.md` + registres front globaux + contrat SELARL comme methode | `docs/sprints/SPRINT_SELAS_FRONT_CONTRACT_001.md` + parcours candidat SELAS medecin associe unique + blocages visibles + prochain `SELAS-TICKETS-001` |
| SELAS-TICKETS-001 | DONE | Produire le backlog ordonne SELAS avant dev | `SPRINT_SELAS_FRONT_CONTRACT_001.md` + matrice SELAS + audit reuse + journal NotebookLM | `docs/sprints/SPRINT_SELAS_TICKETS_001.md` + tickets arbitrage/spec/dev candidat/reserve + premier candidat `SELAS-FRONT-SCHEMA-001` bloque jusqu'a `GO dev` Gad |
| SELAS-GO-DEV-FIRST-TICKET-001 | DONE | Preparer la demande d'arbitrage Gad pour le premier ticket codable | `SPRINT_SELAS_TICKETS_001.md` + contrat front + `NO-GO dev` | `docs/sprints/SPRINT_SELAS_GO_DEV_FIRST_TICKET_001.md` + recommandation `SELAS-FRONT-SCHEMA-001` sans generation DOCX + attente decision Gad |
| SELAS-GAD-FEEDBACK-001 | DONE | Traiter le retour Gad sur cas par cas et reutilisation SELARL/SELAS | reponse Gad transmise par Naomie + audit reuse SELAS | `docs/sprints/SPRINT_SELAS_GAD_FEEDBACK_001.md` + strategie revisee : socle commun SELAS reutilisable, activation documentaire progressive, `NO-GO dev` maintenu |
| SELAS-FRONT-SCHEMA-001 | DONE_PARTIAL_QA | Coder le socle commun SELAS front/data sans generation documentaire | `SPRINT_SELAS_GAD_FEEDBACK_001.md` + `ok go` Gad + contrat front SELAS | role `ACTIONNAIRE`, reuse praticien -> actionnaire/president, module `selas_schema.py`, tests unitaires cibles ; compilation/import/tests manuels OK, `pytest`/`ruff` indisponibles localement |
| SELAS-SPEC-PRESIDENT-001 | DONE | Specifier le role President SELAS avant documents juridiques | `SPRINT_SELAS_TICKETS_001.md` + `SELAS-FRONT-SCHEMA-001` + specs Lot 1/PV gerant | `docs/sprints/SPRINT_SELAS_SPEC_PRESIDENT_001.md` + roles President/Actionnaire/Signataire/Mandataire separes + DG bloque + prochain `SELAS-SPEC-PROCURATION-001` |
| SELAS-SPEC-PROCURATION-001 | DONE | Verrouiller l'adaptation SELAS de `DOC-003` procuration | `SPRINT_SELAS_SPEC_PRESIDENT_001.md` + spec Lot 1 DOC-003 + generateur/tests existants | `docs/sprints/SPRINT_SELAS_SPEC_PROCURATION_001.md` + decision reutiliser `DOC-003` avec fonction `President` stricte + controles anti-gerant/SELARL |
| SELAS-DOC003-PROCURATION-PRESIDENT-001 | DONE_PARTIAL_QA | Coder l'adaptation bornee de `DOC-003` Procuration pour SELAS President unique | `SPRINT_SELAS_SPEC_PROCURATION_001.md` + GO utilisateur + generateur/tests DOC-003 existants | verrou SELAS dans `ProcurationGenerator` : affichage `President`, blocage `Gerant` / `Directeur General`, tests cibles ajoutes ; compilation et smoke manuel OK ; `pytest`/`ruff` indisponibles localement |
| SELAS-SPEC-DNC-PRESIDENT-001 | DONE | Verrouiller la reutilisation SELAS de `DOC-001` DNC President | `SPRINT_SELAS_SPEC_PRESIDENT_001.md` + audit reuse + matrice/front/tickets SELAS + spec Lot 1 DOC-001 + generateur/tests existants | `docs/sprints/SPRINT_SELAS_SPEC_DNC_PRESIDENT_001.md` + decision reutiliser `DOC-001` avec President signataire, filiation obligatoire et blocage si filiation absente |
| SELAS-DOC001-DNC-PRESIDENT-001 | DONE_PARTIAL_QA | Coder l'adaptation bornee de `DOC-001` DNC President pour SELAS President unique | `SPRINT_SELAS_SPEC_DNC_PRESIDENT_001.md` + GO utilisateur + generateur/tests DOC-001 existants + schema SELAS | schema SELAS `DOC-001` aligne sur President/Signataire/filiation/signature ; tests generateur et schema ajoutes ; compilation et smoke manuel OK ; `pytest`/`ruff` indisponibles localement |
| SELAS-SPEC-DOC002-DOMICILIATION-001 | DONE | Verrouiller la reutilisation SELAS de `DOC-002` Domiciliation | audit reuse + matrice/front/tickets SELAS + spec Lot 1 DOC-002 + generateur/tests existants | `docs/sprints/SPRINT_SELAS_SPEC_DOC002_DOMICILIATION_001.md` + decision reutiliser `DOC-002` comme document neutre seulement si domiciliation = siege/cabinet |
| SELAS-DOC002-DOMICILIATION-001 | DONE_PARTIAL_QA | Coder l'adaptation bornee de `DOC-002` Domiciliation pour SELAS President unique | `SPRINT_SELAS_SPEC_DOC002_DOMICILIATION_001.md` + GO utilisateur + generateur/tests DOC-002 existants + schema SELAS | schema SELAS `DOC-002` aligne sur President/Signataire/Societe, capital, siege et reuse siege->domiciliation obligatoire ; tests generateur/schema ajoutes ; compilation et smoke manuel OK ; `pytest`/`ruff` indisponibles localement |
| SELAS-SPEC-DECISION-PRESIDENT-001 | DONE | Verrouiller Decision/PV nomination President SELAS avant tout code | `SPRINT_SELAS_SPEC_PRESIDENT_001.md` + spec `DOC-004` PV nomination gerant + matrice/front/tickets SELAS + schema SELAS | `docs/sprints/SPRINT_SELAS_SPEC_DECISION_PRESIDENT_001.md` + decision `DOC-004` patron structurel seulement + code bloque sans source texte/statuts |
| SELAS-SPEC-STATUTS-MEDECIN-AU-001 | DONE | Verrouiller Statuts SELAS medecin actionnaire unique avant code | specs Lot 4 statuts SEL + arbitrages SEL + source `Statuts_SELAS_medecin.docx` + spec Decision President + schema SELAS | `docs/sprints/SPRINT_SELAS_SPEC_STATUTS_MEDECIN_AU_001.md` + `DOC-018` candidat V1 + nomination President dans les statuts + DG nomme/multi-actionnaires/actions complexes bloques |
| SELAS-SPEC-CAPITAL-ACTIONS-001 | DONE | Verrouiller capital, actions, valeur nominale et numerotation SELAS V1 | spec Statuts SELAS medecin AU + specs Lot 4 statuts SEL + schema SELAS + modele capital existant | `docs/sprints/SPRINT_SELAS_SPEC_CAPITAL_ACTIONS_001.md` + actions ordinaires 100 % actionnaire unique + droits proportionnels + numerotation simple derivee `1 a N` + cas complexes bloques |
| SELAS-SPEC-ORDRE-001 | DONE | Verrouiller la reutilisation de `DOC-034` Demande d'inscription a l'Ordre pour SELAS | specs Lot 2 Ordre + audit reuse + matrice/front/tickets SELAS + spec President + schema SELAS + generateur/tests ordre existants | `docs/sprints/SPRINT_SELAS_SPEC_ORDRE_001.md` + `DOC-034` reutilisable via overlay SELARL/SELAS + President ordinal distinct du President SELAS + plans/devis comme pieces attendues non generees |
| SELAS-SPEC-REGIME-COMMUNAUTAIRE-001 | DONE | Verrouiller `DOC-005` / `DOC-006` regime communautaire SELAS | specs Lot 2 regime communautaire + source SELAS + matrix/front/schema/generator/tests | `docs/sprints/SPRINT_SELAS_SPEC_REGIME_COMMUNAUTAIRE_001.md` + batch conditionnel `DOC-005`/`DOC-006` |
| SELAS-ROADMAP-TO-COMPLETION-001 | DONE | Ecrire l'ordre A-Z jusqu'a cloture SELAS V1 et preparer la validation Gad | sprint SELAS + specs SELAS realisees + demande Naomie/Gad | `docs/sprints/SPRINT_SELAS_ROADMAP_TO_COMPLETION_001.md` + ordre officiel propose + message a envoyer a Gad + `NO-GO dev` maintenu avant validation |
| SELAS-DOC034-ORDRE-001 | DONE_PARTIAL_QA | Brancher `DOC-034` Demande d'inscription a l'Ordre pour SELAS | roadmap validee + spec Ordre SELAS + generateur/tests ordre existants + schema SELAS | `docs/sprints/SPRINT_SELAS_DOC034_ORDRE_001.md` + schema SELAS DOC-034 aligne + tests cibles ajoutes + smoke manuel DOCX OK ; `pytest`/`ruff` indisponibles localement |
| SELAS-DOC005-DOC006-REGIME-COMMUNAUTAIRE-001 | DONE_PARTIAL_QA | Brancher le batch conditionnel `DOC-005` / `DOC-006` Regime communautaire pour SELAS | roadmap validee + spec Regime communautaire SELAS + generateurs/tests regime existants + schema SELAS | `docs/sprints/SPRINT_SELAS_DOC005_DOC006_REGIME_COMMUNAUTAIRE_001.md` + schema SELAS DOC-005/DOC-006 aligne + `DOC-006` sorti de reserve + tests cibles ajoutes + smoke manuel DOCX OK ; `pytest`/`ruff` indisponibles localement |
| SELAS-DOC018-STATUTS-MEDECIN-AU-001 | DONE_PARTIAL_QA | Brancher `DOC-018` Statuts SELAS medecin actionnaire unique | roadmap validee + spec Statuts SELAS medecin AU + spec Capital Actions + generateur/tests statuts SEL existants | `docs/sprints/SPRINT_SELAS_DOC018_STATUTS_MEDECIN_AU_001.md` + verrous capital/actions et cas hors V1 + `DOC-004` non selectionne si nomination President absorbee + tests cibles ajoutes + smoke manuel DOCX OK ; `pytest`/`ruff` indisponibles localement |
| SELAS-FRONT-READINESS-PACK-001 | DONE_PARTIAL_QA | Aligner la readiness front/data du pack SELAS V1 | roadmap validee + docs SELAS unitaires codes + schema SELAS | `docs/sprints/SPRINT_SELAS_FRONT_READINESS_PACK_001.md` + pack simple/conditionnel/reserve expose cote front/data + generation pack toujours inactive + prochain `SELAS-ORCHESTRATOR-PACK-001` ; `pytest`/`ruff` indisponibles localement |
| SELAS-ORCHESTRATOR-PACK-001 | DONE_PARTIAL_QA | Brancher la selection orchestrateur du pack SELAS V1 | readiness pack + docs SELAS unitaires codes + orchestrateur existant | `docs/sprints/SPRINT_SELAS_ORCHESTRATOR_PACK_001.md` + selection simple/conditionnelle alignee sur readiness + cas hors V1 bloques + prochain `SELAS-SMOKE-HAPPY-PATH-001` ; `pytest`/`ruff` indisponibles localement |
| SELAS-SMOKE-HAPPY-PATH-001 | DONE_PARTIAL_QA | Generer et verifier le pack SELAS simple sans regime communautaire | orchestrateur pack + docs SELAS unitaires codes + contexte SELAS medecin actionnaire unique President unique | `docs/sprints/SPRINT_SELAS_SMOKE_HAPPY_PATH_001.md` + artefacts `artifacts/selas_smoke_happy_path_001/20260602_130911/` + 5 DOCX generes + controles placeholders/SELARL/Gerant/parts sociales OK + prochain `SELAS-SMOKE-REGIME-COMMUNAUTAIRE-001` |
| SELAS-SMOKE-REGIME-COMMUNAUTAIRE-001 | DONE_PARTIAL_QA | Generer et verifier le pack SELAS avec regime communautaire | orchestrateur pack + docs SELAS unitaires codes + contexte SELAS medecin actionnaire unique President unique + conjoint regime communautaire | `docs/sprints/SPRINT_SELAS_SMOKE_REGIME_COMMUNAUTAIRE_001.md` + artefacts `artifacts/selas_smoke_regime_communautaire_001/20260602_133412/` + 7 DOCX generes + controles placeholders/SELARL/Gerant/parts sociales/actionnaire OK + prochain `SELAS-ANTI-REGRESSION-WORDING-001` |
| SELAS-ANTI-REGRESSION-WORDING-001 | DONE_PARTIAL_QA | Verifier les regressions de wording SELAS sur les packs smoke | packs happy path + regime communautaire + wording sensible SELAS | `docs/sprints/SPRINT_SELAS_ANTI_REGRESSION_WORDING_001.md` + artefacts `artifacts/selas_anti_regression_wording_001/20260602_134239/` + strict OK sans `SELARL`/`Gerant`/`parts sociales` + occurrences `associe` classees pour checkpoint trois sources |
| SELAS-TRIPLE-SOURCE-CHECK-001 | DONE_PARTIAL_QA | Comparer le pack SELAS V1 aux trois reperes projet avant revue humaine | source metier `Documents_a_generer_par_cas.docx` + journal NotebookLM SELAS + retour Gad disponible + packs smoke | `docs/sprints/SPRINT_SELAS_TRIPLE_SOURCE_CHECK_001.md` + artefacts `artifacts/selas_triple_source_check_001/20260602_135234/` + source metier/NotebookLM alignes sur perimetre V1 + retour humain pack absent + prochain `SELAS-HUMAN-REVIEW-PACK-001` |
| SELAS-HUMAN-REVIEW-PACK-001 | DONE_WAITING_HUMAN_REVIEW | Preparer le pack SELAS V1 pour revue Gad / associe / juriste | packs smoke + checkpoint trois sources + questions ouvertes + pre-check NotebookLM | `docs/sprints/SPRINT_SELAS_HUMAN_REVIEW_PACK_001.md` + artefacts `artifacts/selas_human_review_pack_001/20260602_135708/` + ZIP `artifacts/selas_human_review_pack_001/selas_human_review_pack_001_20260602_135708.zip` + `NOTEBOOKLM_PRECHECK.md` + attente retour humain |
...
| MAIN-NAOMIE-TRIGGER-001 | DONE | Corriger le cas ou un nouveau chat Naomie demarre sur `main` | capture 2026-06-01 : `bonjour` / `je suis naomi` repond de maniere generique sur `main` | fail-safe `main` dans `AGENTS.md` et `PROJECT_CONTROL_TOWER_V1.md`, kit gouvernance pousse sur `main` |
...
- Prochaine etape recommandee : utiliser `docs/project/SELARL_CANONICAL_STATUS_V1.md`, puis lancer la revue humaine du pack simple avant de rouvrir `SELARL-COMPLETE-COMPLEX-SUBFORMS-001`.
...
- Sprint Naomie actif : SELAS, `docs/sprints/SPRINT_SELAS_V1.md`, phase 31
  `HUMAN REVIEW PACK SELAS`, statut `SELAS-HUMAN-REVIEW-PACK-001` realise ;
  pack de revue humaine pret, attente retour Gad / associe / juriste.
...
- Pour l'accueil de Naomie : repondre avec `Statut sprint`, `Action maintenant`,
  `Point pedagogie`, `Prochaine etape`; ne pas relancer NotebookLM 01 si la
  phase courante a avance, et ne pas produire de pack SELAS sans nouvelle
  spec/gate. Prochaine action SELAS : transmettre le pack de revue humaine,
  puis classer les retours humains avant cloture.

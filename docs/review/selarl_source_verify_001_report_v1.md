...
| Formulaire de dérogation pour exercer sur plusieurs sites avec la SEL | Oui, mais présenté comme générable/formulaire à compléter | Oui, `DOC-013` générable | Générable comme formulaire à compléter | Hors génération pilote : non fourni dans les sources variables V2 | Statut trop permissif | Passer le catalogue et les specs en manuel / hors génération pilote | `src/sydel_doc_engine/domain/case_catalog.py`, `tests/unit/test_case_catalog.py`, specs SELARL |
| Dérogation SEL BNC | Oui | Oui, `MANUAL_ONLY` | Manuel | Manuel | Conforme | Aucune correction catalogue | Aucun |
| Dérogation cumul SELARL BNC | Oui, mais présenté comme générable/formulaire à compléter | Oui, `DOC-014` générable | Générable comme formulaire à compléter | Manuel : V2 indique `à remplir à la main` | Statut incompatible avec la V2 | Passer en manuel dans le catalogue et dans les specs | `src/sydel_doc_engine/domain/case_catalog.py`, `tests/unit/test_case_catalog.py`, specs SELARL |

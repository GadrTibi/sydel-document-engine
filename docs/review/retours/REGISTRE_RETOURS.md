...
| R22-06 | mise-en-forme | SCI IRIS : « statuts : respecter la mise en forme du texte du document source » | moteur de rendu civil PARTAGÉ (SCI = SCI IRIS) | **bloqué — précisions Rafael** : le moteur applique déjà titres/articles gras + titre encadré ; SCI (même moteur) acceptée ; from-scratch ratifié (ADR-0004, pas de copie Word au mm). Besoin du pointeur PRÉCIS (quel élément, quel attribut) avant de toucher le rendu partagé |

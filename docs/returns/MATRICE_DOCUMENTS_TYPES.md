...
| DOC-018 | Statuts SELAS médecin (unipersonnelle) | `lot_04/statuts_selas_medecin.py` | `Statuts {dénomination}.docx` (fallback `statuts_selas_medecin.docx`) — R10 Rafael 2026-07-07 | SELAS (front : type dédié « SELAS uni médecin ») | overlay `selas_medecin`, associé unique | **Spécifique** (socle partagé) |
| DOC-046 | Statuts SELAS dentiste (unipersonnelle) | `lot_04/statuts_selas_dentiste.py` | `Statuts {dénomination}.docx` (fallback `statuts_selas_dentiste.docx`) — R10 | SELAS (front : « SELAS uni dentiste ») | overlay `selas_dentiste`, associé unique | **Spécifique** (socle partagé) |
...
| DOC-015 | Statuts SAS / SPFPL médecins | `lot_04/statuts_sas.py` | `Statuts {dénomination}.docx` (fallback `statuts_sas_spfpl_medecins.docx`) — R10 | SAS | type `spfpl_medecins` + profession médecin + actionnaire unique | **Spécifique** |
| DOC-048 | Statuts SASU Holding | `lot_04/statuts_sasu_holding.py` | `Statuts {dénomination}.docx` (fallback `statuts_sasu_holding.docx`) — R10 | SASU_HOLDING | contexte `statuts_sasu_holding` présent | **Spécifique** (DISTINCT de DOC-015, conservés tous deux) |
| DOC-035 | Statuts SPFPL cession | `lot_04/statuts_spfpl_cession.py` (socle `statuts_spfpl_common.py` + `statuts_spfpl_templates.py`) | `Statuts {dénomination}.docx` (fallback `statuts_spfpl_cession.docx`) — R10 | SPFPL cession | `operation_spfpl.type == cession` + `options.cession` | **Spécifique** (socle SPFPL PARTAGÉ avec DOC-036) |
| DOC-036 | Statuts SPFPL apport | `lot_04/statuts_spfpl_apport.py` | `Statuts {dénomination}.docx` (fallback `statuts_spfpl_apport.docx`) — R10 | SPFPL apport | `operation_spfpl.type == apport` + `options.apport` | **Spécifique** (socle partagé) |
...
| DOC-020 | Statuts SCI | `lot_04/statuts_sci.py` | `Statuts {dénomination}.docx` (fallback `statuts_sci.docx`) — R10 | SCI | `statuts_civils.type == sci` | **Spécifique** (socle partagé) |
| DOC-021 | Statuts SCI IRIS | `lot_04/statuts_sci_iris.py` | `Statuts {dénomination}.docx` (fallback `statuts_sci_iris.docx`) — R10 | SCI IRIS | `statuts_civils.type == sci_iris` + associé PM | **Spécifique** (socle partagé) |
| DOC-025 | Statuts SCM | `lot_04/statuts_scm.py` | `Statuts {dénomination}.docx` (fallback `statuts_scm.docx`) — R10 | SCM | `statuts_civils.type == scm` ; 1-6 associés | **Spécifique** (socle partagé) |
| DOC-047 | Statuts micro holding | `lot_04/statuts_micro_holding.py` | `Statuts {dénomination}.docx` (fallback `statuts_micro_holding.docx`) — R10 | MICRO_HOLDING | `statuts_civils.type == micro_holding` ; capital variable (max = 10× min) | **Spécifique** (socle partagé ; modèle officiel Albane 2026-06-29, 26 articles) |

...
| R0702-02 | « le régime matrimonial doit être le même menu déroulant partout, garde le même que sur la SELARL » | Rafael | SPFPL + SAS (seuls divergents /7) | **TRAITÉ + POUSSÉ**. SPFPL (cession+apport) `7bb1f91` — menu complet + conjoint conditionnel + comparution branchée marié/célibataire ; propagation Akainu : acte parts/actions/apport + attestation + statuts branchés via `mentions_conjoint` ; marié byte-identique ; **Akainu 3 rounds → RIEN À REDIRE**. SAS `b64f632` — même alignement (GO Gad) + collapse célibataire post-traité ; **Akainu 2 rounds → RIEN À REDIRE**. | ⬜ |
| R0702-03 | « tout est en police 12 au lieu de 10 ; ajouter de l'espace entre l'en-tête et le cadre des statuts, après l'adresse de l'associé, avant l'article 1 ; l'annexe c'est top » | Albane | statuts civils (injection ; micro-holding testée) | **TRAITÉ + POUSSÉ** `2a20884`. Police 10 forcée sur `new_document_from_model` (docDefaults+Normal ; corps+footer 10pt vérifié) + 3 espacements micro-holding + verrous de test. **Akainu → MINEUR seulement** (verrous M1/m2 ajoutés, n1 propagation SCI tracée Albane). Byte-fidélité SCI/SCM/SCS intacte. **Annexe = VALIDÉE Albane** ✅ | ⬜ |
